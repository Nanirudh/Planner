    ...
    if(empty($cat))
     ...
     echo '<p>Please fill url field</p>';
      ...
      echo '<p>Please insert urlname field</p>';
            ...
            $res="select url from bookmarkhe where emailid='$mailid' and category='$cat' and url='$url'";

            $num=mysqli_num_rows(mysqli_query($con,$res));
            if($num==1)
            echo '<p>Already present</p>';
            else{
            $query="insert into bookmarkhe (emailid,category,url,urlname) values('$mailid','$cat','$url','$urlname')";
...
<form class="form-horizontal" role="form" method="post" action="bookmarkinsert.php">
